...
methods* methods_make() {
    ...
    newmethods->_TM_size = &_TM_size;
    newmethods->_TM_add = &_TM_add;
    newmethods->_TM_iter = &_TM_iter;
    newmethods->_TM_del = &_TM_del;
    newmethods->_TM_chain_dump = &_TM_chain_dump;
    newmethods->_TM_tree_dump = &_TM_tree_dump;
    newmethods->_TM_get = &_TM_tree_get;
    newmethods->_iter_make = &_iter_make;
    return newmethods;
}
...
int main(void) {
    ...
    my_TM->methods->_TM_add(my_TM, "h", 22);
    my_TM->methods->_TM_add(my_TM, "h", 42); // Replace (更新)
    my_TM->methods->_TM_add(my_TM, "d", 8);
    my_TM->methods->_TM_add(my_TM, "b", 123);
    my_TM->methods->_TM_add(my_TM, "f", 6);

    my_TM->methods->_TM_chain_dump(my_TM);

    printf("2. More Insertions (k, m, j) - Filling gaps\n");
    my_TM->methods->_TM_add(my_TM, "k", 9);
    my_TM->methods->_TM_add(my_TM, "m", 67);
    my_TM->methods->_TM_add(my_TM, "j", 12);

    printf("\n3. Edge Cases (Insert smaller than head, insert larger than tail)\n");
    my_TM->methods->_TM_add(my_TM, "a", 1); // New Head (比 b 小)
    my_TM->methods->_TM_add(my_TM, "z", 999); // New Tail (比 m 大)

    my_TM->methods->_TM_size(my_TM);

    my_TM->methods->_TM_chain_dump(my_TM);
    // 最终顺序: a -> b -> d -> f -> h -> j -> k -> m -> z

    my_TM->methods->_TM_tree_dump(my_TM);

    printf("\n4. Iter print test\n");
    int i = 0;
    my_TM->_iter = my_TM->methods->_iter_make(my_TM);

    while (my_TM->_iter->_current) {
        i++;
        printf("%d key: %s, value: %d\n", i, my_TM->_iter->_current->key, my_TM->_iter->_current->value);
        my_TM->methods->_TM_iter(my_TM);
    }
    my_TM->_iter->_del(my_TM->_iter);

    printf("\n5. Lookup Test\n");
    printf("Value of 'h': %d\n", my_TM->methods->_TM_get(my_TM, "h", -1));
    printf("Value of 'z': %d\n", my_TM->methods->_TM_get(my_TM, "z", -1));
    printf("Value of 'x' (missing): %d\n", my_TM->methods->_TM_get(my_TM, "x", -1));

    printf("\n6. destroy MT\n");
    my_TM->methods->_TM_del(my_TM);
