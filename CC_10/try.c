...
void _node_connect(TM* o_TM, _node* curr, _node* new_node, int direction_int, _node* left_neighbor, _node* right_neighbor) {
    ...
    if (left_neighbor != NULL && right_neighbor != NULL) {
        //      1）左右邻居都不为NULL → new_node 夹在中间        new_node->_prev = left_neighbor;
...
void _TM_add(TM* self, char* key_add, int value_add) {
    ...
    if (self->_root == NULL) {
        ...
        self->_head = new_node;
        self->_tail = new_node;
        printf("_TM_add function is over → key:%s, value:%d\n", key_add, value_add);
        return;
    } // 如果树为空 → 新节点为树根 → 任务完成
...
void _iter_del(TM* self) {
    if (self->_iter != NULL) {
        free(self->_iter);
        self->_iter = NULL;
        printf("iter 内存已释放，并将 self->iter 赋值 NULL\n");
        return;
    } else {
        printf("self->iter 本身就为 NULL\n");
        return;
    }
}
...
_node* _TM_iter(TM* self) {
    ...
    if (self->_iter == NULL) {
        // 从未使用过迭代器，需要新建迭代器
        self->_iter = _iter_make(self);
    }
    if (self->_iter->_current == NULL) {
        return NULL;
    }
    // 迭代器已经存在，需要获得当前curr，并将curr指向下一个
    ret_node = self->_iter->_current;
...
void _TM_del(TM* self) {
    ...
    free(self->_methods);
    free(self);
    printf("TM destroy success\n");
    return;
}

_methods* _methods_make() {
    _methods* new_methods = (_methods*)malloc(sizeof(*new_methods));
    new_methods->_TM_size = &_TM_size;
    new_methods->_TM_add = &_TM_add;
    new_methods->_TM_iter = &_TM_iter;
    new_methods->_TM_del = &_TM_del;
    new_methods->_TM_chain_dump = &_TM_chain_dump;
    new_methods->_TM_tree_dump = &_TM_tree_dump;
    new_methods->_TM_get = &_TM_tree_get;
    return new_methods;
}
...
int main(void) {
    ...
    my_TM->_methods->_TM_add(my_TM, "h", 22);
    my_TM->_methods->_TM_add(my_TM, "h", 42); // Replace (更新)
    my_TM->_methods->_TM_add(my_TM, "d", 8);
    my_TM->_methods->_TM_add(my_TM, "b", 123);
    my_TM->_methods->_TM_add(my_TM, "f", 6);

    my_TM->_methods->_TM_chain_dump(my_TM);

    printf("2. More Insertions (k, m, j) - Filling gaps\n");
    my_TM->_methods->_TM_add(my_TM, "k", 9);
    my_TM->_methods->_TM_add(my_TM, "m", 67);
    my_TM->_methods->_TM_add(my_TM, "j", 12);

    printf("\n3. Edge Cases (Insert smaller than head, insert larger than tail)\n");
    my_TM->_methods->_TM_add(my_TM, "a", 1); // New Head (比 b 小)
    my_TM->_methods->_TM_add(my_TM, "z", 999); // New Tail (比 m 大)

    my_TM->_methods->_TM_size(my_TM);

    my_TM->_methods->_TM_chain_dump(my_TM);
    // 最终顺序: a -> b -> d -> f -> h -> j -> k -> m -> z

    my_TM->_methods->_TM_tree_dump(my_TM);

    printf("\n4. Iter print test\n");
    int i = 1;
    while (1) {
        _node* TM_iter = my_TM->_methods->_TM_iter(my_TM);
        if (TM_iter == NULL) {
            my_TM->_iter->_del(my_TM);
            break;
        }
        printf("%d key: %s, value: %d\n", i, TM_iter->key, TM_iter->value);
        i++;
    }

    printf("\n5. Lookup Test\n");
    printf("Value of 'h': %d\n", my_TM->_methods->_TM_get(my_TM, "h", -1));
    printf("Value of 'z': %d\n", my_TM->_methods->_TM_get(my_TM, "z", -1));
    printf("Value of 'x' (missing): %d\n", my_TM->_methods->_TM_get(my_TM, "x", -1));

    printf("\n6. destroy MT\n");
    my_TM->_methods->_TM_del(my_TM);
